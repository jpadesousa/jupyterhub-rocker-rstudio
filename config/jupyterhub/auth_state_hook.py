...
def auth_state_hook(spawner, auth_state):
    ...
    spawner.environment.update({'USERID': str(uid), 'GROUPID': str(gid), 'NB_UID': str(uid), 'NB_GID': str(gid)})
    spawner.log.info(f"Setting USERID and NB_UID to {uid} and GROUPID and NB_GID to {gid} for user {spawner.user.name}")
